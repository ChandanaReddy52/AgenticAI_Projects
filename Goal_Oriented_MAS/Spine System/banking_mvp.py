...
TEST_QUERIES = [
        {
            "query": "The amount got deducted but the payment failed.",
            "expected_primary": "transaction"
        },
        {
            "query": "I see a transaction I did not make.",
            "expected_primary": "fraud"
        },
        {
            "query": "Someone used my card without my permission.",
            "expected_primary": "fraud"
        },
        {
            "query": "My card is lost, please block it.",
            "expected_primary": "card"
        },
        {
            "query": "I want to know my loan EMI schedule.",
            "expected_primary": "loan"
        },
        {
            "query": "I cannot log into my account.",
            "expected_primary": "account_access"
        },
        {
            "query": "Payment is pending since yesterday.",
            "expected_primary": "transaction"
        },
        {
            "query": "Why is my balance not updated?",
            "expected_primary": "transaction"
        }
    ]
...
if __name__ == "__main__":
    ...
    for idx, item in enumerate(TEST_QUERIES, start=1):
        ...
        # ---- Intent ----
        intent_raw = intent_classification_task.output.raw
        intent_dict = safe_json_load(intent_raw)

        intent_validated = validate_intent_output(
            intent_dict,
            user_query=user_query
        )

        # ---- Policy ----
        policy_raw = policy_reasoning_task.output.raw or "{}"
        policy_validated = validate_policy_output(
            safe_json_load(policy_raw)
        )

        # ---- Final Decision ----
        final_decision = decide_resolution(
            intent=intent_validated,
        ...
        record = {
            ...
            "resolution": final_decision["resolution_mode"],
    ...
    # NOTE:
    # Response Drafting Agent output is intentionally not wired yet.
