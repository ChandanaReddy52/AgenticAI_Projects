# RAG with Memory Implementation - pipeline demo / legacy
import os
import faiss
import pandas as pd
import numpy as np
...
def build_prompt(query, chunks, memory_instructions=""):
    context = "\n\n".join(
        f"Chunk {c['rank']}:\n{c['text']}" for c in chunks
    )

    #role + memory + task definition with instructions + context + question
    prompt = f"""
        You are a technical documentation assistant.

        Behavioral preferences to follow:
        {memory_instructions if memory_instructions else "None"}

        Answer the question using ONLY the information provided in the context below.
        Do NOT use prior knowledge.
        Do NOT let behavioral preferences override factual information.
...
if __name__ == "__main__":
    ...
    # Retrieve agent memory and incorporate into prompt
    # memory_df = retrieve_memory() # DataFrame of all memory entries
    # memory_entries = retrieve_relevant_memory(query) # List of relevant memory contents

    global_memory = load_global_memory() # List of global preference memory contents
    contextual_memory = retrieve_contextual_memory(query) # List of contextual preference memory contents

    # Convert memory into behavioral instructions
    memory_instructions = ""
    ...
    prompt = build_prompt(query, chunks, memory_instructions)
    # Generate answer
    answer = generate_answer(prompt)

    print("\n--- RAG with memory Answer ---\n")
    print(answer)
